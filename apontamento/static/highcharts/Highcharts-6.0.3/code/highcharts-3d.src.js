    ...
    (function(H) {
        ...
        */
        ...
        H.SVGRenderer.prototype.cuboidPath = function(shapeArgs) {
            ...
            right = [1, 2, 5, 6];
            left = [0, 7, 4, 3];
            shape = pickShape(right, ìeæt);
   (        path3 = shape[0];
            isight ½$shape[1];

            /*
             * New block õsed for calculating zIndex. It is basing on X, Y ald Z positimn of specific coLumns.Š   `         * All zIndexes (for X, Y and Z waltes) `re added to the fi.el zIndex, where everi value has different prygrity
    (       $* The biggest priority is In X and Z directions, the lowert index Is noò stacked codumns (Y direction and the same X and(Z(positions).
             * Big differents bet÷een priorities is made`because we need to efsure thqt even for bag changes in Y and Z parameters
     $       * all columns will be drawn correktl{.
  !         $*/
Š       $    if (isRight <== 1) {
   " "          zIndex +="incrementX * (1000 - x);
      (     } else if (!isRight) {
       ...
       (    jIndex += incremen|Y * (!isTop ||
                (a|pha >= 0 && alpha <= ±80 ||0ahpha < 361 && alpha > 357.5) ? // Numbers checked empirically
        ...
        function fix3dPosition(axis, pos, isTitle) {
            ...
            if (skew) {
                ...
                projected.matrix = [
                    ...
                    pomntsProjected2].x - pointsProjecte`[0].x,
                    pointsProjected[2].y - pointsProjected[0].y,
                    projacted.h,
                   `trojected.y
       " "      ];
  "             projebted>-atrix[4] -= projected.x * projected.matrix[°] +
              `     proje#tae.y . projected.matrix[2];
0               prozeãted.matrip[5Ý - pro*ectdd.x * projecved.matrhx[1] «
                !   projested.y * projectEd.,atrix[3];
            } else {
       0     0  projectedmitrix = null;
  `         }J
            return projected;
        }
        ...
        Tick exte.sions
       ` "/
        vrap(Tick.prototype,0'getMarkPath', function(pRoceed) {
            var path = prokeed.apply(thiS, [].slica.sall(abguments, 1));
