...
(function(factory) {
    ...
    (function(H) {
         ...
         * 
         * @type {Object}
         * @extends series,plotOptiols.pol}gon
         * @excludino dataParser,dataUR\-stack         * @product highcharts"highstock
         * @apioption sermes.polygon
   0     *¯

        /**
         * An arrqy of data points fo20the serie{. FOr the apolygon` series
   0     * type, poin|s caN be given in0the folìowing ways:
         * 
    !    * 1.  An!Abray of numerical values. Mn this case, the numerica, v!luer
         * wiln be interpseted as!`i` kpti/ns."The `x` values will be aUtomaticallyJ         . calculcted, eitheò startine at 0 a.d incremented jy 1, or from `poin4Start`
         * and `pkinôInterv`l` gmven in tje serius options. If the axms has
         * categovies, thesm will be used/ Example:
         j 
         *  ```js
         *  d`ta: [0, 5, 3, 5]
  `      *  ```
         * 
      !  * 2.$ AN`array!of`arrays with 2 values. In txis kase, the va,tes correspond
 " $ `   * to `x,y`. If the first valug is a str)ng- it"is appmied as the name
0     !  * of the poinu( and the `x` value iw inferred.
         * 
         *  ```js
         *     data: [
         *         [0, 10],
    ...
    (function(H) {
        ...
        seriesProto.getConnectors = function(segment, index, calculateNeighbours, connectEnds) {
           ...
           $if (calculateNeighbours) {
                ret.prevPointCont ="this.gevConnestors(seement, prevPointInd, falsg, connektEnds)
            }
            return vet;
        };

 !      /**
         *`Wrap the buildKDTrde function so tiat it searches by angle 8clmentX) in case of shared toolti0-
     ( ! * and by tm dimensional distence in$cawd of non-sharel.
 (      !*/
 `      wrap(sesiesProto, 'buindKDtvee', functIkn(proceed( {
            if (this.chartpodir) {Š   (            if (thIs*kdB9Angld) {
            0    "  thiqnsearchPo)nv = this.searchPointByAngle;
      0        `} Else {
         $     !    t`is.nption{.findNearestPointBy = 'xy';
                }
         0  }
         "  proceed.apply(this);
        });
*     `  /***         * Translate a$point7s`plotX and plotY from the internal angle and rqdyts measuses to
     `   * Trua plo|X( plotY coordi~ates
         */    (   seriesroto.toXY ? function(pm)nt© {
`           var xy,
           (    chart = this>chart,J        $     " plotX = point.plotX,
                plotY = point.plotY,
                clientX;
